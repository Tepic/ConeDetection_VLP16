function cones_location = processLIDAR_(LIDAR,rowrow)
...
%                     LIDAR_close = pointCloud([x,y,z]);
                    ...
                    % examined point
                    [indices,dists] = findInRadius_(LIDAR(locs,:),[xx yy zz],.5,points,1);
                    ...
                    if(zz<0.2 && sqrt(xx^2+yy^2)>middle_boundry && sqrt(xx^2+yy^2)<outter_boundry && abs(yy)<y__outter_boundry)
                        ...
                        [indices,dists] = findInRadius_(data_set,[xx yy zz],1,points,0);
